...
Write-Host "Start: MFHelper Automated Test Suite" -ForegroundColor Cyan
...
# Run tests
Write-Host "[RUN] Running automated tests..." -ForegroundColor Yellow
Write-Host ""

# Check if node_modules exists
if (-not (Test-Path "tests/node_modules")) {
    Write-Host "[INSTALL] Installing test dependencies..." -ForegroundColor Yellow
    Push-Location tests
    npm install
    Pop-Location
...
Write-Host "[RUN] Running Playwright E2E tests..." -ForegroundColor Cyan
Push-Location tests
